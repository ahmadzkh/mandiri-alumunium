...
|---|---|---|---|---|---|---|---|---|
| CP-001 | Approve documentation baseline before implementation | None | Approved `PRD.md`, `DESIGN.md`, `CLAUDE.md`, `AGENTS.md`, `TASK.md` | All required docs exist, required sections present, user approves implementation start | M | P0 | Pending | Read files, heading audit, user approval |
| CP-002 | Scaffold Next.js App Router project | CP-001 | Next.js project with TypeScript, App Router, TailwindCSS | Project boots locally; no Pages Router; no backend scaffold | M | P0 | Pending | `npm run dev`, browser smoke |
| CP-003 | Configure shadcn/ui baseline | CP-002 | shadcn/ui configuration and initial primitives | shadcn/ui works with Tailwind theme; no unnecessary components installed | S | P0 | Pending | Import/use one selected primitive, lint/build |
| CP-004 | Set up quality scripts | CP-002 | Scripts for lint, typecheck, build, test/e2e when configured | `package.json` exposes stable verification commands; commands do not conflict | M | P0 | Pending | Run lint/typecheck/build |
| CP-005 | Define project folder structure | CP-002 | Approved folders for app routes, components, data, lib, assets, tests | Structure is minimal, readable, and aligned with static-first architecture | S | P0 | Pending | File tree review |
